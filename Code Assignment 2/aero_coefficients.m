function [dL,C_L,dD_i,C_D_i,alpha_i,alpha_eff,C_N,C_T,w_ind,u_magnitude,dQ,dQ_cumulative,C_Q,C_Q_cumulative,dT] = aero_coefficients(N,R,rho,chordlength,surface_area,chord,u_inflow,u_freestream,circulation,B_ind,alpha,alphadata,Clspline,Cdspline,panel_width,cp_spanwise)
...
for i = 1:length(circulation)
    alpha_i(i) = -rad2deg(atan(w_ind(i)/u_magnitude(i)));
    ...
    if alpha_eff(i) <= min(alphadata);
        C_L(i) = ppval(Clspline,min(alphadata));
    end
    if alpha_eff(i) > min(alphadata) && alpha_eff(i) < max(alphadata)
